...
///
/// Linux port: the OS context is reported as Linux (Warp ships a Linux client),
/// otherwise identical to the macOS app.
enum WarpProvider {
    ...
    static func fetch() async -> ProviderSnapshot {
        ...
        let body: [String: Any] = [
            ...
            "variables": ["requestContext": [
                "clientContext": [:],
                "osContext": ["category": "Linux", "name": "Linux", "version": osString],
            ]],
        ]
        ...
        request.setValue("Linux", forHTTPHeaderField: "x-warp-os-category")
        request.setValue("Linux", forHTTPHeaderField: "x-warp-os-name")
