...
enum DroidProvider {
    ...
    static func loadAPIKey() -> String? {
        ...
        let url = URL(fileURLWithPath: "\(Paths.home)/.factory/.env")
    ...
    private static func fetchBillingLimits(key: String) async -> ProviderSnapshot? {
        guard let (data, response) = try? await HTTP.data(for: request(limitsURL, key: key)),
    ...
    private static func fetchLegacyUsage(key: String) async -> ProviderSnapshot {
        ...
        guard let (data, response) = try? await HTTP.data(for: request(usageURL, key: key)) else {
    ...
    private static func fetchPlan(key: String) async -> String? {
        guard let (data, response) = try? await HTTP.data(for: request(authURL, key: key)),
              (response as? HTTPURLResponse)?.statusCode == 200,
              let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
