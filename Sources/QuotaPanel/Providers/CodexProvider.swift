...
enum CodexProvider {
    ...
    /// Loaded at runtime from ~/.quotapanel/oauth-clients.json — never committed.
    ...
    static func fetch() async -> ProviderSnapshot {
        ...
        } else if let fileCreds = loadCredentials() {
            creds = fileCreds
        } else {
        ...
        if creds.isExpired {
            if !creds.persistRefresh, let cached = await refreshed.valid() {
                creds = Credentials(
                ...
                return snapshot(.authProblem("Token expired and refresh failed — sign in from Settings or run `codex` once"))
            }
        }
    ...
    private static func refreshAccessToken(_ creds: Credentials) async -> Credentials? {
        ...
        do {
            ...
            if creds.persistRefresh {
                ...
                stored.accountId = stored.accountId ?? creds.accountId
            ...
            return Credentials(
                ...
                persistRefresh: creds.persistRefresh
            )
        } catch {
            return nil
        }
